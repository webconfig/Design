...
namespace BehaviorDesigner.Editor
{
	...
	public class TaskList : ScriptableObject
	{
		...
		public void init()
		{
			...
			this.Search(BehaviorDesignerUtility.SplitCamelCase(this.mSearchString).ToLower().Replace(" ", ""), this.mCategoryList);
		}

		public void addTasksToMenu(BehaviorDesignerWindow window, ref GenericMenu genericMenu)
		{
			this.addCategoryTasksToMenu(window, ref genericMenu, this.mCategoryList);
		}

		private void addCategoryTasksToMenu(BehaviorDesignerWindow window, ref GenericMenu genericMenu, List<TaskList.CategoryList> categoryList)
		{
			for (int i = 0; i < categoryList.Count; i++)
			{
				...
				if (categoryList[i].Tasks != null)
				{
					...
					{
						genericMenu.AddItem(new GUIContent(string.Format("Add Task/{0}/{1}", categoryList[i].Fullpath, BehaviorDesignerUtility.SplitCamelCase(categoryList[i].Tasks[j].Type.Name.ToString()))), false, new GenericMenu.MenuFunction2(window.addTaskCallback), categoryList[i].Tasks[j].Type);
					}
				}
			}
		}
		...
		public void drawTaskList(BehaviorDesignerWindow window)
		{
            //搜索
		...
		private bool Search(string searchString, List<TaskList.CategoryList> categoryList)
		{
			...
			for (int i = 0; i < categoryList.Count; i++)
			{
				...
				if (BehaviorDesignerUtility.SplitCamelCase(categoryList[i].Name).ToLower().Replace(" ", "").Contains(searchString))
				{
					result = true;
					flag = true;
					categoryList[i].Visible = true;
				...
				if (categoryList[i].Tasks != null)
				{
					for (int j = 0; j < categoryList[i].Tasks.Count; j++)
					{
						...
						if (flag || BehaviorDesignerUtility.SplitCamelCase(categoryList[i].Tasks[j].Type.Name).ToLower().Replace(" ", "").Contains(searchString))
						{
